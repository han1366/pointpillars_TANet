...
class KittiViewer(QMainWindow):
    ...
    def on_loadButtonPressed(self):
        ...
        #print("infos:",self.kitti_infos[0])
    ...
    def sample_to_current_data(self):
        ...
        sampled_difficulty = []
        # class_names = ["Car"]
        pc_info = self.kitti_info
        #calib = self.kitti_info["calib"]
        rect = pc_info['calib/R0_rect']
        P2 = pc_info['calib/P2']
        Trv2c = pc_info['calib/Tr_velo_to_cam']
        num_features = 4
        if 'pointcloud_num_features' in pc_info:
            num_features = pc_info['pointcloud_num_features']
    ...
    def draw_gt_in_image(self):
        ...
        calib_info = self.kitti_info
        rect = calib_info['calib/R0_rect']
        P2 = calib_info['calib/P2']
        Trv2c = calib_info['calib/Tr_velo_to_cam']
    ...
    def plot_pointcloud(self):
        ...
        idx = self.image_idxes.index(pc_info["image_idx"])
        if 'annos' in self.kitti_info:
    ...
    def load_info(self, image_idx):
        ...
        image = None
        if 'img_path' in pc_info:
            img_path = pc_info['img_path']
            if img_path != "":
        ...
        self.points = points
        rect = pc_info['calib/R0_rect'].astype(np.float32)
        Trv2c = pc_info['calib/Tr_velo_to_cam'].astype(np.float32)
        P2 = pc_info['calib/P2'].astype(np.float32)
        image_shape = None
        if 'image_shape' in pc_info:
            image_shape = pc_info['img_shape']
